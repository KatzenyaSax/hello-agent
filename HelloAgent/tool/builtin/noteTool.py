...
class NoteTool(Tool):
    ...
    # ------------------------------------------------------------------
    # 程序化访问接口（供 Agent 代码直接调用，非 LLM 路径）
    # ------------------------------------------------------------------

    def search_notes_structured(self, query: str, note_type: str = None, limit: int = 10) -> List[Dict[str, Any]]:
        """结构化搜索笔记，返回笔记 dict 列表

        Args:
            query: 搜索关键词（空字符串则匹配所有）
            note_type: 按类型过滤（可选）
            limit: 返回数量限制

        Returns:
            笔记 dict 列表，字段: id/title/content/type/tags/created_at/updated_at/metadata
        """
        query_lower = (query or "").lower()
        matched = []

        for idx_note in self.notes_index["notes"]:
            if note_type and idx_note.get("type") != note_type:
                continue
